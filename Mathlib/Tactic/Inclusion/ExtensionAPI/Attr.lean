...
private def analyzeTheoremArgs (declName : Name) (pattern outputSet : Expr)
  ...
  for h : i in [:args.size] do
    ...
    if let some (inputExpr, inputSet, _) := toSetMem? argType then
      ...
      if (registeredParams.find? userName).isSome then
        unless ← isDefEq argType (mkConst ``Nat) do
          throwError "Inclusion parameter '{userName}' in '{declName}' does not have type `Nat`"
